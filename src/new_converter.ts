...
export interface PathReplacementRule {
    enabled: boolean;
    actorType: PathType;
    arrayIndex: number | "Any";
    name?: string;
    originalPath: string;
    newPath: string;
}
...
function applyPathReplacements(
    ...
    const state = GlobalStore.getState() as any;
    const rules: PathReplacementRule[] = state?.pathReplacementRules ?? [];

    if (!rules || rules.length === 0) return path;

    for (const rule of rules) {
        if (!rule.enabled) continue;

        if (rule.actorType !== "Any" && rule.actorType !== type) continue;
        if (rule.arrayIndex !== "Any" && rule.arrayIndex !== index) continue;

        if (path === rule.originalPath) {
            return rule.newPath;
        }
    }
